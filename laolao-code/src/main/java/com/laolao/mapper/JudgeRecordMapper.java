...
public interface JudgeRecordMapper extends BaseMapper<JudgeRecord> {
    ...
    @Select("""
            ...
            SELECT jmr.best_judge_record_id as id,
                   ...
                   jr.score   as member_score,
                   q.standard_solution,
                   jr.answer_code,
                   jr.status,
                   ai.content as ai_report
            FROM judge_member_result jmr
    ...
    @Select("""
            ...
            FROM question q
                     ...
                     LEFT JOIN judge_member_result jmr ON jmr.question_id = q.id AND jmr.user_id = #{userId}
    ...
    @Select("""
            ...
            FROM exam e
                     ...
                     JOIN judge_member_result jmr ON jmr.exam_record_id = er.id
                     JOIN judge_record jr ON jr.id = jmr.best_judge_record_id
