...
public interface JudgeRecordMapper extends BaseMapper<JudgeRecord> {
    ...
    @Select("select question_id, status, score, error_message, total_count, pass_count, failed_input, failed_expect, failed_actual, time, memory from judge_record where id = #{judgeRecordId}")
