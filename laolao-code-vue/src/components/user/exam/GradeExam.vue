        ...
        <div class="flex flex-col flex-1 items-center p-2 space-y-2 overflow-y-auto">
            ...
            <div class=" w-5/7 shrink-0 border rounded py-2 px-6 space-y-2 bg-white">
                <div v-for="judgeRecord in currentUser?.judgeRecords" :key="judgeRecord.id">
                    ...
                    <div class="h-96">
                        <MonacoEditor v-if="currentUser" v-model="judgeRecord.answerCode" language="java" theme="vs" />
                    </div>
...
<script setup lang="ts">
    ...
    const getUserAnswer = async () => {
        try {
            const res = await axios.get("/api/exam/grade/user-answer", {
                params: {
                    examRecordId: currentUser.value?.id
                }
            })
            currentUser.value!.judgeRecords = res.data.data
        } catch (e) {
            console.log(e);
        }
    }
